...
export default function Modal({ open, title, description, children, onClose, size = 'lg' }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/50 p-4 pt-[6vh] backdrop-blur-[2px]"
      onMouseDown={(event) => {
    ...
    >
      <div
        ...
        tabIndex={-1}
        className={`mb-8 w-full ${sizes[size] || sizes.lg} overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl outline-none motion-safe:animate-[kinetic-dialog-in_160ms_ease-out]`}
      >
        ...
        <div className="max-h-[78vh] overflow-y-auto p-5 md:p-6">{children}</div>
      </div>
    </div>
  );
}
