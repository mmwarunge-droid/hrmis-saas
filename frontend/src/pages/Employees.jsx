...
import { Building2, Grid2X2, List, Network, Plus, Search, UsersRound } from 'lucide-react';
  ...
  const columns = [
    {
      key: 'full_name',
      label: 'Person',
      render: (row) => <Link to={`/employees/${row.id}`} className="flex items-center gap-3"><Avatar name={row.full_name} size="sm" /><div><p className="font-semibold text-slate-900">{row.full_name}</p><p className="text-xs text-slate-500">{row.email}</p></div></Link>,
    },
    ...
    <div className="space-y-7">
      ...
      <Card className="space-y-4">
        <div className="grid gap-3 lg:grid-cols-[1fr_190px_220px_auto]">
          <div className="relative"><Search className="pointer-events-none absolute left-3 top-3 text-slate-400" size={18} /><Input aria-label="Search people" className="pl-10" placeholder="Search people, roles or locations" value={query} onChange={(event) => setQuery(event.target.value)} /></div>
          <Select aria-label="Employment status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option><option value="active">Active</option><option value="probation">Probation</option><option value="suspended">Suspended</option><option value="terminated">Terminated</option></Select>
          <Select aria-label="Department" value={department} onChange={(event) => setDepartment(event.target.value)}><option value="">All departments</option>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</Select>
          <div className="flex rounded-2xl bg-slate-100 p-1"><Button size="sm" variant={view === 'cards' ? 'primary' : 'ghost'} onClick={() => setView('cards')}><Grid2X2 size={16} /></Button><Button size="sm" variant={view === 'list' ? 'primary' : 'ghost'} onClick={() => setView('list')}><List size={16} /></Button></div>
        </div>
      ...
      {loading ? <div className="h-64 animate-pulse rounded-3xl bg-slate-100" /> : filtered.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((employee) => (
            <Link key={employee.id} to={`/employees/${employee.id}`} className="group">
              <Card className="h-full transition duration-200 group-hover:-translate-y-1 group-hover:border-cyan-200 group-hover:shadow-xl">
                ...
                <div className="mt-5 grid grid-cols-2 gap-3 text-xs">
                  <div className="rounded-2xl bg-slate-50 p-3"><p className="text-slate-400">Department</p><p className="mt-1 truncate font-semibold text-slate-700">{departmentNames[employee.department_id] || 'Unassigned'}</p></div>
                  <div className="rounded-2xl bg-slate-50 p-3"><p className="text-slate-400">Location</p><p className="mt-1 truncate font-semibold text-slate-700">{employee.work_location || 'Not set'}</p></div>
                </div>
