...
def _write_demo_pdf_document(
    ...
    pdf.drawString(72, page_height - 90, title)
...
def _seed_documents(tenant, employees, users, reference: date):
    ...
    for index, (
        ...
        is_native_signing_demo = key == 'information-security'
        extension = 'pdf' if is_native_signing_demo else 'txt'
        stored_filename = f'demo-{index:02d}-{key}.{extension}'
        original_filename = f'{key}.{extension}'
        content = (
            f'{title}\n\n'
            'This is fictional demonstration content generated by Kinetic.\n'
            'It contains no real employee, customer or legal information.\n'
        )
        if is_native_signing_demo:
            stored = _write_demo_pdf_document(
                tenant.id,
                stored_filename,
                title,
            )
...
def _seed_goals(tenant, employees, departments, users, reference: date):
    specs = (
        (
            'organization-revenue',
            'Reach the quarterly recurring-revenue target',
            'Grow recurring revenue while preserving healthy implementation capacity.',
            'organization',
            None,
            None,
            Decimal('1200000'),
            Decimal('840000'),
            'KES',
            reference - timedelta(days=35),
            reference + timedelta(days=55),
            'active',
            'on_track',
            users['owner'],
        ),
        (
            ...
            'Ship the employee onboarding refresh',
            'Complete the redesigned onboarding journey and production rollout.',
            'department',
            None,
            departments['product'],
            Decimal('20'),
        ...
        (
            'department-people-completion',
            'Lift first-week onboarding completion',
            'Increase first-week completion across required employee tasks.',
            'department',
            None,
            departments['people'],
            Decimal('95'),
            Decimal('82'),
            '%',
            reference - timedelta(days=40),
            reference + timedelta(days=20),
            'active',
            'on_track',
            users['consultant'],
        ),
        (
            'neema-strategic-opportunities',
            'Close 18 strategic opportunities',
            'Convert qualified opportunities within the current sales cycle.',
            'employee',
            employees['neema-hassan'],
            None,
            Decimal('18'),
            Decimal('11'),
            'opportunities',
            reference - timedelta(days=31),
            reference + timedelta(days=47),
            'active',
            'on_track',
            users['manager-growth'],
        ),
        (
            'kevin-first-month',
            'Complete the first 30-day success plan',
            'Finish role onboarding, stakeholder introductions, and the first design brief.',
            'employee',
            employees['kevin-mwangi'],
            None,
            Decimal('10'),
            Decimal('6'),
            'milestones',
            reference - timedelta(days=9),
            reference + timedelta(days=21),
            'active',
            'on_track',
            users['consultant'],
        ),
        (
            'brian-pipeline',
            'Build KES 400M in qualified pipeline',
            'Create sufficient qualified coverage for the next two quarters.',
            'employee',
            employees['brian-mutua'],
            None,
            Decimal('400'),
            Decimal('220'),
            'KES millions',
            reference - timedelta(days=42),
            reference + timedelta(days=18),
            'active',
            'at_risk',
            users['owner'],
        ),
        (
            'faith-vendor-savings',
            'Deliver KES 8M in vendor savings',
            'Renegotiate priority vendor contracts without reducing service levels.',
            'employee',
            employees['faith-wekesa'],
            None,
            Decimal('8'),
            Decimal('5.5'),
            'KES millions',
            reference - timedelta(days=48),
            reference + timedelta(days=12),
            'active',
            'on_track',
            users['owner'],
        ),
        (
            'engineering-incident-actions',
            'Close reliability action items',
            'Complete the action plan from the prior reliability review.',
            'department',
            None,
        ...
        (
            'neema-certification',
            'Complete enterprise discovery certification',
            'Finish the assessed discovery programme and practical review.',
            'employee',
            employees['neema-hassan'],
            None,
            Decimal('6'),
    ...
    for index, spec in enumerate(specs):
        ...
        progress = min(
            Decimal('100'),
            (current_value / target_value * Decimal('100')).quantize(Decimal('0.01')),
        )
        goals[key] = _upsert(
            Goal,
            f'goal:{key}',
            ...
            weight=Decimal('100'),
            last_check_in_at=_at(reference - timedelta(days=index % 6), 15),
            created_at=_at(start_date, 9),
            updated_at=_at(reference - timedelta(days=index % 4), 15),
        )

    check_in_specs = (
        ('org-revenue-1', 'organization-revenue', users['owner'], Decimal('640000'), 'Initial enterprise renewals secured.'),
        ('org-revenue-2', 'organization-revenue', users['owner'], Decimal('840000'), 'Pipeline conversion remains on plan.'),
        ('product-1', 'department-product-launch', users['consultant'], Decimal('8'), 'Core employee journey approved.'),
        ('product-2', 'department-product-launch', users['consultant'], Decimal('12'), 'Integration work is behind the original plan.'),
        ('people-1', 'department-people-completion', users['consultant'], Decimal('76'), 'Manager reminders introduced.'),
        ('people-2', 'department-people-completion', users['consultant'], Decimal('82'), 'Completion improved after the new reminder cadence.'),
        ('neema-1', 'neema-strategic-opportunities', users['employee'], Decimal('8'), 'Three opportunities moved to commercial review.'),
        ('neema-2', 'neema-strategic-opportunities', users['employee'], Decimal('11'), 'Two strategic accounts reached final negotiation.'),
        ('kevin-1', 'kevin-first-month', users['new-hire'], Decimal('3'), 'Profile, security, and stakeholder introductions complete.'),
        ('kevin-2', 'kevin-first-month', users['new-hire'], Decimal('6'), 'First design brief submitted for review.'),
        ('brian-1', 'brian-pipeline', users['manager-growth'], Decimal('180'), 'Partner pipeline is below plan.'),
        ('brian-2', 'brian-pipeline', users['manager-growth'], Decimal('220'), 'Two enterprise opportunities qualified.'),
        ('faith-1', 'faith-vendor-savings', users['manager-operations'], Decimal('5.5'), 'Cloud and facilities renewals completed.'),
        ('engineering-1', 'engineering-incident-actions', users['client-admin'], Decimal('9'), 'Remaining actions require engineering ownership.'),
        ('certification-1', 'neema-certification', users['employee'], Decimal('6'), 'Certification completed and manager validated.'),
    )
    for index, (key, goal_key, actor, current_value, note) in enumerate(check_in_specs):
        goal = goals[goal_key]
        progress = min(
            Decimal('100'),
            (current_value / goal.target_value * Decimal('100')).quantize(Decimal('0.01')),
        )
        _upsert(
            GoalCheckIn,
            f'goal-check-in:{key}',
            ...
            health=goal.health,
            note=note,
            created_at=_at(reference - timedelta(days=14 - min(index, 13)), 15),
            updated_at=_at(reference - timedelta(days=14 - min(index, 13)), 15),
        )
    return goals
...
def _seed_demo_data(
    *,
    ...
    _seed_goals(primary, employees, departments, users, reference_date)
    _seed_employee_home(
        primary,
        documents,
        users,
